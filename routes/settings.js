...
router.post("/switchServerAccess",async (req,res) =>{
	const {machineId} = await db.Machine_info.findOne({where:{machineId:req.body.matchID}})
	if(machineId != null || machineId != undefined)
	{
		const {ngrok_port} = await db.Server_data.findOne({where:{id:1}})
		const newport = ngrok_port == 22 ? process.env.PORT : 22
		await db.Server_data.update({ngrok_port:newport},{where:{id:1}})
		exec("sudo reboot", (err) => {
			console.log(err);
			if (!err) return res.status(200).send("success");
		});
